 ...
 * 功能：用户店铺中心-发布产品
 */
class UproductAction extends Action{
    ...
    function lists(){
        ...
        $m = M("goods");
        $where = array('create_by' => $_SESSION['Username']);
        ...
        //分页显示输出
        $show = $page->show();
        $data = $m->order("selltime desc, id")->where($where)->select();
        R('Base/header');
        R('Base/footer');
    ...
    */
    function inse(){
        ...
        $_POST['zuixin'] = 1;
        $_POST['create_by'] = $_SESSION['Username'];
    ...
    */
    function update(){
        ...
        $m = M("goods");
        $_POST['gdescription']=stripslashes($_POST['gdescription']);
